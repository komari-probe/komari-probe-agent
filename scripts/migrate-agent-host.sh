#!/usr/bin/env bash
# Migrate one host's Agent to Sonar. Server hosts use the Server repo script.
set -u -o pipefail

REPO="sonar-probe/sonar-agent"; TAG="latest"; SERVICE="sonar-agent"; BINARY=""; CONFIG=""
BACKUP_ROOT="/var/backups/sonar-agent-migration"; BINARY_URL=""; CHECKSUM_URL=""; CLEANUP_ID=""; DRY_RUN=0
TMP_DIR=""; BACKUP_DIR=""
...
log() { printf '[sonar-agent-migrate] %s\n' "$*"; }
die() { printf '[sonar-agent-migrate] ERROR: %s\n' "$*" >&2; exit 1; }
...
detect_binary() { local p; for p in /opt/sonar-agent/sonar-agent /opt/sonar/agent /opt/sonar/sonar-agent /opt/komari/agent /opt/komari/komari-agent /usr/local/bin/sonar-agent /usr/local/bin/komari-agent; do [ -x "$p" ] && { printf '%s' "$p"; return; }; done; return 1; }
...
BINARY=${BINARY:-$(detect_binary || true)}; [ -n "$BINARY" ] || die "Could not detect old Agent binary; pass --binary PATH."; [ -x "$BINARY" ] || die "Old Agent binary not found: $BINARY"; if ! systemctl status "$SERVICE" >/dev/null 2>&1 && systemctl status "komari-agent" >/dev/null 2>&1; then SERVICE="komari-agent"; fi; systemctl status "$SERVICE" >/dev/null 2>&1 || die "Agent service not found: $SERVICE"
ASSET="sonar-agent-linux-$(arch)"; BASE=$(release_base); BINARY_URL=${BINARY_URL:-"$BASE/$ASSET"}; CHECKSUM_URL=${CHECKSUM_URL:-"$BASE/checksums.txt"}; BACKUP_ID=$(date -u +%Y%m%dT%H%M%SZ)
