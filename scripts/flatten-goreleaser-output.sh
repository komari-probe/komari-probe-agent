...
# each one out to the flat sonar-agent-<os>-<arch>[.exe] layout the rest of
...
done < <(grep -oE '"name":"sonar-agent-[^"]+","path":"[^"]+"' "$BUILD_DIR/artifacts.json" | sed -E 's/"name":"([^"]+)","path":"([^"]+)"/\1 \2/')

find "$BUILD_DIR" -mindepth 1 -maxdepth 1 -type d -name "sonar-agent_*" -exec rm -rf {} +
rm -f "$BUILD_DIR/artifacts.json" "$BUILD_DIR/config.yaml" "$BUILD_DIR/metadata.json" "$BUILD_DIR/checksums.txt"

(
  cd "$BUILD_DIR"
  sha256sum sonar-agent-* > checksums.txt
)
