...
```powershell
python .\tools\fake-agent\fake_agent.py --server https://komari.example.com --adkey YOUR_AUTO_DISCOVERY_KEY -new
```

The generated client token is saved in `fake-agent-client.json` alongside the script. Subsequent runs reuse it:

```powershell
python .\tools\fake-agent\fake_agent.py --server https://komari.example.com --adkey unused
```
